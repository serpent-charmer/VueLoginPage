...
<template>
    <div :class="['account-row', isLdap ? 'account-ldap' : '']">
        <sl-input :class="{ 'field-error': errors?.includes('tags') }"  :value="account.tagsText" @blur="updateAccount(account.uid, {tagsText: $event.target.value})"></sl-input>
        ...
        <sl-input :class="{ 'field-error': errors?.includes('login') }" :value="account.login" @blur="updateAccount(account.uid, {login: $event.target.value})"></sl-input>
        <sl-input v-if="!isLdap" :class="{ 'field-error': errors?.includes('password') }" :value="account.password" type="password" password-toggle @blur="updateAccount(account.uid, {password: $event.target.value})"></sl-input>
